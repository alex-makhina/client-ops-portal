...
export const PersonalAccountPage: React.FC = () => {
    ...
    if (isError) return <div className="dashboard-wrapper"><div className="error-state">{(error as Error).message}</div></div>;
    ...
    return (
        <div className="dashboard-wrapper">
            <div className="dashboard-container">
                ...
                <div className="section-header"><h2 className="section-title">Данные абонента</h2></div>
                <div className="profile-grid">
                    <div className="profile-item"><div className="profile-label">ФИО</div><div className="profile-value">{[abonent.lastName, abonent.firstName, abonent.middleName].filter(Boolean).join(' ')}</div></div>
                    <div className="profile-item"><div className="profile-label">Лицевой счет</div><div className="profile-value">{abonent.accountNumber}</div></div>
                    <div className="profile-item"><div className="profile-label">Идентификатор</div><div className="profile-value">{abonent.identificationNumber}</div></div>
                    <div className="profile-item"><div className="profile-label">Клиент с</div><div className="profile-value">{new Date(abonent.createdAt).toLocaleDateString('ru-RU')}</div></div>
                </div>
                ...
                {showCreateForm && (
                    <div className="action-panel" style={{ marginBottom: 16 }}>
                        <h4 style={{ margin: '0 0 12px', fontSize: 14, color: 'var(--text-primary)' }}>Новый договор</h4>
                        <form onSubmit={handleSubmitContract(onCreateContractSubmit)}>
                            <div className="action-row">
                                <div>
                                    ...
                                    {contractErrors.contractNumber && <div style={{ color: 'var(--btn-danger-text)', fontSize: 11, marginTop: 4 }}>{contractErrors.contractNumber.message}</div>}
                                </div>
                                <div>
                            ...
                            <div className="action-btn-group" style={{ marginTop: 16 }}>
                            ...
                            <div key={contract.id} className={`contract-row ${isExpanded ? 'expanded' : ''}`}>
                                <div className="contract-main" onClick={() => toggleExpand(contract.id)}>
                                    ...
                                    <div className="contract-meta">
                                        <span className="subs-badge">Подключено услуг: {contract.subs.length}</span>
                                        <span className="expand-icon">▼</span>
                                        {!isTerminated &&
                                            <button className="sub-action-btn danger"
                                                onClick={(e) => handleTerminateContract(e, contract.id)}
