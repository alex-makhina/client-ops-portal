...
public class AuthDbSeeder
{
    ...
    public async Task SeedAsync(CancellationToken ct = default)
    {
        ...
        if (await _userManager.FindByNameAsync("admin") == null)
        {
            ...
            if ((await _userManager.CreateAsync(adminUser, "Admin@123456")).Succeeded) { await _userManager.AddToRoleAsync(adminUser, "Admin"); _logger.LogInformation("Admin created"); }
            var sm = new ApplicationUser { UserName = "smanager", Email = "smanager@clientopsportal.com", EmailConfirmed = true };
            if ((await _userManager.CreateAsync(sm, "Smanager@123456")).Succeeded) await _userManager.AddToRoleAsync(sm, "ServiceManager");
            var mgr = new ApplicationUser { UserName = "manager", Email = "manager@clientopsportal.com", EmailConfirmed = true };
            if ((await _userManager.CreateAsync(mgr, "Manager@123456")).Succeeded) await _userManager.AddToRoleAsync(mgr, "Manager");
        }
    }
}