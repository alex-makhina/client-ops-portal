...
namespace ClientOpsPortal.Application.Services
{
    public class ReportsService : IReportsService
    {
        ...
        public ReportsService(IReportsRepository repository, IDirectoryCacheService cache)
        {
        ...
        private async Task EnrichAsync(IEnumerable<Domain.Entities.Subscription> items, CancellationToken ct)
        {
            foreach (var s in items)
            {
                var svc = await _cache.GetServiceAsync(s.ServiceId, ct);
                if (svc != null)
                    s.Service = new Domain.Entities.Service { Id = svc.Id, Name = svc.Name, Description = svc.Description ?? string.Empty, BeginDate = svc.BeginDate, EndDate = svc.EndDate };
                var tp = await _cache.GetTariffPlanAsync(s.TariffPlanId, ct);
                if (tp != null)
                    s.TariffPlan = new Domain.Entities.TariffPlan { Id = tp.Id, Name = tp.Name, Description = tp.Description ?? string.Empty, Price = tp.Price, ServiceId = tp.ServiceId, BeginDate = tp.BeginDate, EndDate = tp.EndDate };
            }
        }
    }
}
